...
class vrd_module():
    ...
    def relation_im(self, im_path, res):
        ...
        print("relscore time: ", time.time()-time1)
        print("relscore shape: ", rel_score.shape)

        print("rel_so_prior shape", rel_so_prior.shape)
        print("spatialfea shape", SpatialFea.shape)
        ...
        # print(vrd_res)
        time2 = time.time()
        print("TEST Time : {}".format(time.strftime('%H:%M:%S', time.gmtime(int(time2 - time1)))))
        return vrd_res
...
def det_im(im_path):
    # randomly generates faster-rcnn format results    
    img_size = (224,224)

    res = {}
    res['cls'] = np.random.randint(0, 100, 10)
    res['confs'] = np.random.rand(10)
    
    # res['box'] --> list of [xmin, ymin, xmax, ymax]
    # xmax > xmin, ymax > ymin
    res['box'] = []
    for i in range(10):
        xmin = np.random.randint(0, img_size[0]-1)
        ymin = np.random.randint(0, img_size[1]-1)
        xmax = np.random.randint(xmin+1, img_size[0])
        ymax = np.random.randint(ymin+1, img_size[1])
        res['box'].append([xmin, ymin, xmax, ymax])
    res['box'] = np.array(res['box'])
