...
class VrdDataLayer(object):
    ...
    def __init__(self, ds_name, stage, model_type = None, proposals_path = None, data_dir=None):
        ...
        self._classes = [x.strip() for x in open(osp.join(self.data_dir,f'{ds_name}/obj.txt')).readlines()]
        self._relations = [x.strip() for x in open(osp.join(self.data_dir,f'../data/{ds_name}/rel.txt')).readlines()]
        ...
        with open(osp.join(self.data_dir,f'{ds_name}/{stage}.pkl'), 'rb') as fid:
        ...
        # print("Annotation[0] :",self._anno[0])

        with open(osp.join(self.data_dir,f'{ds_name}/so_prior.pkl'), 'rb') as fid:
            u = pickle._Unpickler(fid)
            u.encoding = 'latin1'
            self._so_prior = u.load()  
...
if __name__ == '__main__':    
    ...
    blob, boxes, rel_boxes, SpatialFea, classes, ix1, ix2, anno_img = loader.forward()
